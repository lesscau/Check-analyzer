...
from get import getReceiptJson
import unittest, json

class getReceiptJsonTesting(unittest.TestCase):
    ...
    def testGetCorrectReceipt(self):
        ...
        newJson = getReceiptJson(self.fn, self.fd, self.fp, self.login, self.passw)
    ...
    def testUseWrongFn(self):
        ...
        with self.assertRaises(ValueError) as cm: getReceiptJson(fnBad, self.fd, self.fp, self.login, self.passw)
        self.assertEqual('Non-JSON response: "the ticket was not found"', str(cm.exception))

    def testUseWrongFd(self):
        fdBad = "25423"
    
        with self.assertRaises(ValueError) as cm: getReceiptJson(self.fn, fdBad, self.fp, self.login, self.passw)
        self.assertEqual('Non-JSON response: "the ticket was not found"', str(cm.exception))

    def testUseWrongFp(self):
        fpBad = "1627615565"
    
        with self.assertRaises(ValueError) as cm: getReceiptJson(self.fn, self.fd, fpBad, self.login, self.passw)
        self.assertEqual('Non-JSON response: "the ticket was not found"', str(cm.exception))
    ...
    def testUseWrongLogin(self):
        ...
        for login in loginBad:
            with self.subTest(login = login):
                with self.assertRaises(ValueError) as cm: getReceiptJson(self.fn, self.fd, self.fp, login, self.passw)
    ...
    def testUseWrongPassword(self):
        ...
        with self.assertRaises(ValueError) as cm: getReceiptJson(self.fn, self.fd, self.fp, self.login, passwBad)
        self.assertEqual('Non-JSON response: "the user was not found or the specified password was not correct"', str(cm.exception))
